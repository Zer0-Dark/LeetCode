let threeSumClosest = function (nums, target) {
    ...
    for (let i = 0; i < nums.length - 2; i++) {
        for (let j = 1; j < nums.length - 1; j++) {
            for (let k = 2; k < nums.length; k++) {
                if (i < j && j < k) {
                    if(!results.includes(nums[i] + nums[j] + nums[k])){
    ...
    if(results.includes(target)) return target;
    // if it's bigger than the biggest return the biggest
    if(target > results[results.length-1]) return results[results.length-1];
    // if it's smaller than the smallest return the smallest
    if(target < results[0]) return results[0];
    // compare it to the target
    let more = 0;
    for (j = 0; ; j++) {
        for (i = 0; i < results.length; i++) {
            if (results[i] + more === target) {
                return results[i];
            } else if (results[i] - more === target) {
                return results[i];
            }
        }
        more++;
